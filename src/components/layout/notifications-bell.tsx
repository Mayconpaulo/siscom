...
export function NotificationsBell({ initialNotifications, mobile = false }: { initialNotifications: SiscomNotification[]; mobile?: boolean }) {
  ...
  function notificationHref(item: SiscomNotification) {
    if (item.event_id) return `/dashboard/agenda/${item.event_id}`;
    if (item.demand_id) return `/dashboard/demandas/${item.demand_id}`;
    return "/dashboard";
  }
