...
export function FirstAccessForm({ warName }: { warName: string }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!response.ok) { setError(json.error); setLoading(false); return; }
  ...
  return <form onSubmit={submit} className="space-y-4">
    ...
    <Button className="w-full bg-emerald-950" disabled={loading}>{loading && <LoaderCircle size={18} className="animate-spin" />}Concluir cadastro e entrar</Button>
  </form>;
}
