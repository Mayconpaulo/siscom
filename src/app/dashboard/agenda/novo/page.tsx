...
export default async function NewEventPage({ searchParams }: { searchParams: Promise<{ modo?: string }> }) {
  ...
  const [demandResult, profileResult] = supabase ? await Promise.all([
  ...
  return <div className="min-h-dvh bg-slate-50 p-4 pt-20 sm:p-8 lg:pt-8"><div className="mx-auto max-w-3xl">
    ...
    <Card><CardContent className="p-5 sm:p-6"><EventForm demands={demandResult.data || []} profiles={profileResult.data || []} mode={isCompletedMode ? "realizado" : "planejado"} /></CardContent></Card>
  </div></div>;
}
