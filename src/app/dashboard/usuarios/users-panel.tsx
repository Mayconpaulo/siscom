...
export function UsersPanel() {
  ...
  async function load() {
    try {
      ...
      const json = await readApiResponse<{ error?: string; users?: User[] }>(response);
      if (!response.ok) setError(json.error || "Não foi possível carregar os usuários.");
  ...
  async function createUser(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const json = await readApiResponse<{ error?: string; message?: string }>(response);
      if (!response.ok) setError(json.error || "Não foi possível criar o usuário.");
      else { setMessage(json.message || "Usuário criado."); form.reset(); setTemporaryPassword(""); await load(); }
    } catch (reason) {
  ...
  async function update(user: User) {
    ...
    try {
      ...
      const json = await readApiResponse<{ error?: string; message?: string }>(response);
      if (!response.ok) setError(json.error || "Não foi possível atualizar o usuário.");
      else { setMessage(json.message || "Perfil atualizado."); await load(); }
    } catch (reason) {
  ...
  async function sendPasswordReset(user: User) {
    ...
    try {
      ...
      const json = await readApiResponse<{ error?: string; message?: string }>(response);
      if (!response.ok) setError(json.error || "Não foi possível enviar a redefinição.");
      else setMessage(json.message || "Link de redefinição enviado.");
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "Não foi possível enviar a redefinição.");
    }
  }
  ...
  return <div className="space-y-6">
    <form onSubmit={createUser} className="rounded-2xl border bg-white p-5 shadow-sm">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_auto_auto]">
        ...
        <Button type="button" variant="outline" onClick={() => setTemporaryPassword(createTemporaryPassword())} className="w-full"><KeyRound size={16} />Gerar senha</Button>
        <Button disabled={sending} className="w-full bg-emerald-950">{sending ? "Criando..." : "Criar usuário"}</Button>
      </div>
