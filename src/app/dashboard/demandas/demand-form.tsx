...
export function DemandForm({ profiles, demand }: { profiles: ProfileOption[]; demand?: Demand }) {
  ...
  return <form action={action} className="space-y-5">
    ...
    <div className="grid gap-5 sm:grid-cols-2"><div><label className="mb-2 block text-sm font-semibold">Prazo</label><Input name="due_at" type="datetime-local" defaultValue={localDateTime(demand?.due_at)} /></div><div><label className="mb-2 block text-sm font-semibold">Responsável</label><select name="assigned_to" defaultValue={demand?.assigned_to || ""} className="h-11 w-full rounded-lg border bg-background px-3.5 text-sm"><option value="">Não definido</option>{profiles.map((profile) => <option key={profile.id} value={profile.id}>{[profile.rank, profile.war_name || profile.full_name].filter(Boolean).join(" ")}</option>)}</select></div></div>
    <label className="flex cursor-pointer items-center gap-3 rounded-xl border bg-muted/30 p-4"><input type="checkbox" name="completed" defaultChecked={demand?.status === "concluida"} className="size-5 accent-emerald-800" /><span><span className="block text-sm font-semibold">Demanda concluída</span><span className="block text-xs text-slate-500">Marque quando o trabalho estiver finalizado.</span></span></label>
    <div><label className="mb-2 block text-sm font-semibold">Descrição *</label><textarea name="description" required rows={7} defaultValue={demand?.description} className="w-full rounded-lg border bg-background p-3.5 text-sm outline-none focus:border-emerald-700" placeholder="Descreva a necessidade, público e entregáveis..." /></div>
